...
export async function GET({ url, locals }: { url: URL; locals: App.Locals }) {
  ...
  if (all) {
    ...
    const rows = await dbAll(
      env.DB,
      `SELECT * FROM docs ${where} ORDER BY order_index ASC, created_at DESC LIMIT ? OFFSET ?`,
      status ? [status, pageSize, offset] : [pageSize, offset],
    )
    const totalRow = await dbGet<{ total: number }>(
      env.DB,
      `SELECT COUNT(*) as total FROM docs ${where}`,
      status ? [status] : [],
    )
  ...
  const rows = await dbAll(
    env.DB,
    'SELECT * FROM docs WHERE status = ? ORDER BY order_index ASC, published_at DESC LIMIT ? OFFSET ?',
